...
## P14 — 앱 UI·UX 개선 (2026-09-20)

이미 돌아가는 시트가 있다면 아래만 하면 된다.

### a. 코드와 시트
1. `git pull origin main` → `cd apps-script` → `clasp push`
2. 편집기에서 `Setup.gs` 의 `setupSheet` 을 한 번 실행한다. `Assets` 마지막 열 뒤에 `kind` 가 붙고 드롭다운이 걸린다.
   웹앱 자산 화면은 이 열이 없으면 안내를 띄우고, 있으면 종류를 저장한다.
3. 웹앱은 `main` 에 올라가면 자동 배포된다. 홈 화면 앱은 한 번 완전히 닫았다 열면 새 버전이 된다.

### b. 자산을 종류별로 넣기
더보기 › 자산 › **새 스냅샷**. 첫 스냅샷이면 계좌·주식·국민연금·퇴직연금 IRP·401k·HSA 줄이 미리 깔려 있다.
잔액만 채우고 통화(원화 자산은 KRW)를 맞춘 뒤 저장한다. 다음 달부터는 직전 스냅샷의 계좌 목록이
그대로 불려 오므로 잔액만 고치면 된다. 비상금(고정비 N개월치)은 **계좌 잔액(kind=cash)** 만으로 센다.

### c. 로그인이 자주 뜨는 문제
서버가 없어서 구글은 이 앱에 1시간짜리 액세스 토큰만 준다(갱신 토큰은 브라우저 앱에 주지 않는다).
그 안에서 이렇게 줄였다.
- 토큰을 기기에 남겨 두어(localStorage) 1시간 안에는 앱을 닫았다 열어도 아무것도 묻지 않는다.
- 1시간이 지나면 창이 잠깐 떴다 저절로 닫힌다. 예전에는 사람이 "연결" 을 누르면 늘 동의 화면이 떴는데, 이제는 30일에 한 번만 뜬다.
- 처음 연결한 계정을 기억해 계정 선택 화면을 건너뛴다. 로그아웃하면 잊는다.
- 앱을 막 열었을 때는 브라우저가 팝업을 막을 수 있다. 그때는 오류 대신 "연결" 버튼이 달린 안내가 뜨고, 한 번 누르면 된다.

동의 화면이 여전히 매번 뜬다면 Google Auth Platform 의 게시 상태가 "테스트" 이고 계정이 테스트 사용자에 있는지 본다.
완전히 묻지 않게 하려면 Apps Script 웹 앱을 데이터 중계로 두어야 하는데, 그러면 "서버 없음" 원칙과 시트 공유 = 접근 권한 원칙이 깨진다. ADR 로 따로 정한다.

### d. 달라진 화면
- 원장 탭은 달력으로 열린다. 달력을 좌우로 밀거나 ‹ › 를 누르면 달이 넘어간다.
- 수입 탭에서 급여·레슨을 누르면 그 달에 들어온 기록이 펼쳐지고, 항목을 누르면 고칠 수 있다.
